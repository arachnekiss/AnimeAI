...
class MotionGenerator:
    ...
    def generate_emotion_animation(self, emotion: str, intensity: float = 1.0) -> Dict:
        """
        Generate emotion-based animation data
        
        Args:
            emotion: Emotion type (happy, sad, surprised, etc.)
            intensity: Emotion intensity (0.0 to 1.0)
        
        Returns:
            Dict containing animation data for the emotion
        """
        try:
            logger.info(f"Generating emotion animation: {emotion} (intensity: {intensity})")
            ...
            # Get emotion mapping
            emotion_params = self.emotion_mappings.get(emotion_type, {})
            ...
            animation_data = {
                ...
                'intensity': intensity,
                'duration': self.default_emotion_transition_time,
                'facial_parameters': scaled_params,
                'body_adjustments': {
                    'posture': scaled_params.get('body_posture', 'neutral'),
                    'head_tilt': scaled_params.get('head_tilt', 0.0),
                    'breathing_rate': self.breathing_rate * (1.0 + intensity * 0.2)
                },
                'keyframes': self._generate_emotion_keyframes(emotion_type, intensity)
            }
            
            logger.info(f"Generated emotion animation data for {emotion}")
            return animation_data
            
        except Exception as e:
            logger.error(f"Error generating emotion animation: {e}")
            return self._get_fallback_emotion_data(emotion, intensity)
    
    def generate_gesture_animation(self, gesture: str, duration: float = 2.0) -> Dict:
        """
        Generate gesture-based animation data
        
        Args:
            gesture: Gesture type (wave, nod, shrug, etc.)
            duration: Animation duration in seconds
        
        Returns:
            Dict containing animation data for the gesture
        """
        try:
            logger.info(f"Generating gesture animation: {gesture} (duration: {duration}s)")
            ...
            try:
                ...
                logger.warning(f"Unknown gesture: {gesture}, using NONE")
            
            # Generate gesture-specific animation data
            animation_data = {
                'gesture_type': gesture_type.value,
                'duration': duration,
                'body_motion': self._generate_gesture_motion(gesture_type, duration),
                'facial_motion': self._generate_gesture_facial(gesture_type),
                'timing': {
                    'start_time': 0.0,
            ...
            logger.info(f"Generated gesture animation data for {gesture}")
            return animation_data
            
        except Exception as e:
            logger.error(f"Error generating gesture animation: {e}")
            return self._get_fallback_gesture_data(gesture, duration)
    
    def generate_conversation_animation(self, message: str, response: str) -> Dict:
        """
        Generate conversation-based animation combining message analysis and response
        ...
        Returns:
            Dict containing animation data for the conversation
        """
        try:
            logger.info(f"Generating conversation animation for message: '{message[:50]}...'")
            
            # Analyze conversation context
            emotion = self._analyze_conversation_emotion(message, response)
            gestures = self._analyze_gesture_context(response)
            duration = self._calculate_duration(response)
            
            # Generate comprehensive animation
            animation_data = {
                'conversation_type': 'dialogue',
                'message': message,
                'response': response,
                'duration': duration,
                'emotion': emotion.value if emotion else EmotionType.NEUTRAL.value,
                'gestures': [g.value for g in gestures] if gestures else [],
                'lip_sync': self._generate_lip_sync_data(response),
                'body_animation': self._generate_conversation_body_motion(response, emotion),
                'facial_animation': self._generate_conversation_facial_motion(response, emotion),
                'timing': {
                    'listening_phase': 0.5,  # Brief pause before responding
                    'speaking_phase': duration - 0.5,
                    'total_duration': duration
                }
            }
            
            logger.info(f"Generated conversation animation for {len(response.split())} words")
            return animation_data
            
        except Exception as e:
            logger.error(f"Error generating conversation animation: {e}")
            return self._get_fallback_conversation_data(message, response)
    
    def _generate_emotion_keyframes(self, emotion_type: EmotionType, intensity: float) -> List[Dict]:
        """Generate keyframes for emotion transitions"""
        keyframes = []
        
        # Simple emotion keyframe generation
        for i in range(3):  # Start, peak, end
            time = i * (self.default_emotion_transition_time / 2)
            blend_value = intensity if i == 1 else 0.0  # Peak at middle frame
            
            keyframes.append({
                'time': time,
                'emotion_blend': blend_value,
                'facial_blend': emotion_type.value
            })
        
        return keyframes
    
    def _generate_gesture_motion(self, gesture_type: GestureType, duration: float) -> Dict:
        """Generate body motion for specific gestures"""
        motion_data = {
            'arms': [],
            'hands': [],
            'head': [],
        ...
        # Simple gesture motion patterns
        if gesture_type == GestureType.WAVE:
            motion_data['arms'] = [
                {'time': 0.0, 'right_arm_rotation': [0, 0, 0]},
                {'time': duration * 0.3, 'right_arm_rotation': [-45, 30, 0]},
                {'time': duration * 0.7, 'right_arm_rotation': [-45, -30, 0]},
                {'time': duration, 'right_arm_rotation': [0, 0, 0]}
            ]
        elif gesture_type == GestureType.NOD:
            motion_data['head'] = [
                {'time': 0.0, 'head_rotation': [0, 0, 0]},
                {'time': duration * 0.5, 'head_rotation': [15, 0, 0]},
                {'time': duration, 'head_rotation': [0, 0, 0]}
            ]
        # Add more gesture patterns as needed
        
        return motion_data
    
    def _generate_gesture_facial(self, gesture_type: GestureType) -> Dict:
        """Generate facial expressions for gestures"""
        facial_data = {}
        
        if gesture_type == GestureType.WAVE:
            facial_data = {'mouth_smile': 0.3, 'eye_squint': 0.1}
        elif gesture_type == GestureType.SHRUG:
            facial_data = {'eyebrow_raise': 0.4, 'mouth_slight': 0.2}
        
        return facial_data
    
    def _analyze_conversation_emotion(self, message: str, response: str) -> EmotionType:
        """Analyze emotion from conversation context"""
        # Simple emotion analysis based on keywords
        text = (message + " " + response).lower()
        
        if any(word in text for word in ['happy', 'great', 'wonderful', 'excellent']):
            return EmotionType.HAPPY
        elif any(word in text for word in ['sad', 'sorry', 'unfortunate', 'bad']):
            return EmotionType.SAD
        elif any(word in text for word in ['wow', 'amazing', 'incredible', 'surprising']):
            return EmotionType.SURPRISED
        elif any(word in text for word in ['think', 'consider', 'analyze', 'wonder']):
            return EmotionType.THINKING
        else:
            return EmotionType.NEUTRAL
    
    def _generate_lip_sync_data(self, text: str) -> Dict:
        """Generate lip synchronization data for speech"""
        return {
            'phonemes': [],  # Would be populated by actual lip sync analysis
            'mouth_shapes': [],
            'timing': [],
            'text': text
        }
    
    def _generate_conversation_body_motion(self, text: str, emotion: EmotionType) -> Dict:
        """Generate body motion for conversation"""
        return {
            'posture': 'attentive',
            'gestures': self._analyze_gesture_context(text),
            'breathing_pattern': 'normal'
        }
    
    def _generate_conversation_facial_motion(self, text: str, emotion: EmotionType) -> Dict:
        """Generate facial motion for conversation"""
        return {
            'emotion_blend': emotion.value,
            'lip_sync': self._generate_lip_sync_data(text),
            'eye_contact': True,
            'blink_pattern': 'natural'
        }
    
    def _get_fallback_emotion_data(self, emotion: str, intensity: float) -> Dict:
        """Fallback emotion data in case of errors"""
        return {
    ...
    def _get_fallback_gesture_data(self, gesture: str, duration: float) -> Dict:
        """Fallback gesture data in case of errors"""
        return {
    ...
    def _get_fallback_conversation_data(self, message: str, response: str) -> Dict:
        """Fallback conversation data in case of errors"""
        return {
            'conversation_type': 'dialogue',
            'message': message,
            'response': response,
